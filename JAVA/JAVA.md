...
| key = map.keySet().iterator().next()               | **获取第一个键**<br>1. **`map.keySet()`**：返回Map中所有key的Set集合<br>2. **`.iterator()`**：获取这个Set的迭代器<br>3. **`.next()`**：获取迭代器的第一个元素（即第一个key）                                 | 对于`HashMap`，返回的key顺序是**不确定的**：<br>对于有序的Map，顺序是确定的：LinkedHashMap保持插入顺序,TreeMap按key排序                                                       |
| freqToDummy.computeIfAbsent(freq, k -> newList()); | 如果key不存在，则使用提供的函数创建value并放入Map；如果key存在，则直接返回对应的value。                                                                                                              | - **`freqToDummy`**：一个Map，假设类型是 `Map<Integer, ListNode>`<br>- **`freq`**：要查找的key<br>- **`k -> newList()`**：Lambda表达式，当key不存在时执行，创建新的value |
