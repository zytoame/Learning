...
AOF 重写的核心思想是：**基于当前数据库的内存数据，逆向生成一系列可以重建当前数据状态的最小命令集，并写入一个新的 AOF 文件**。

它**不是**对旧的 AOF 文件进行读取、分析或压缩，而是直接遍历数据库中的所有键值对。

#### 1. 重写过程

1. **主进程 `fork` 出一个子进程**来进行重写操作。
    
    - 使用子进程是为了避免阻塞主进程，保证 Redis 的高性能。
        
    - `fork()` 操作会产生一个与父进程（主进程）**共享同一份内存数据**的子进程。这一刻的内存快照成为了重写的基础。
        
2. **子进程遍历数据库**，根据不同类型和大小的键，选择最优化的命令写入新的临时 AOF 文件。
    
    - 对于一个字符串键，直接生成一条 `SET` 命令。
        
    - 对于一个列表、集合、哈希或有序集合，如果元素数量较少，可能会生成一条完整的创建命令（如 `RPUSH mylist a b c`）。
        
    - 对于元素数量非常大的键，Redis 会采用一种“微优化”，可能会拆分成多条命令，但最终目的还是为了用最精简的方式重现数据。
        
3. **在重写期间，主进程依然正常处理客户端请求**。所有新的写命令不仅会追加到**原有的 AOF 缓冲区**，还会同时追加到一个特殊的 **AOF 重写缓冲区**。这是为了确保重写开始后产生的新数据变更不会丢失。
    
4. **子进程完成重写**后，会向父进程发送一个信号。
    
5. **父进程（主进程）接收到信号后**，会进行以下操作：
    
    - 将 **AOF 重写缓冲区**中的所有内容追加到新的临时 AOF 文件中。这一步会阻塞主进程，但通常很快，因为缓冲区内的数据量一般不大。
        
    - 原子性地用新的 AOF 文件**替换**掉旧的 AOF 文件（`rename` 系统调用）。


### 注意事项

- **`fork` 操作的阻塞**：虽然重写本身在子进程进行，但 `fork()` 这个瞬间操作本身可能会阻塞主进程，尤其是在虚拟机或 Redis 内存使用量非常大的情况下。`fork` 需要复制页表，内存越大，页表越大，阻塞时间可能越长。
    
- **重写期间的磁盘压力**：重写过程需要大量磁盘 I/O 来写入新文件。如果同时开启 `appendfsync always` 策略，磁盘压力会很大，可能会影响 Redis 性能。可以考虑将重写安排在业务低峰期。
    
- **重复重写**：在重写期间，如果又有大量新数据写入，可能导致重写完成后新的 AOF 文件立刻又超过了自动重写的阈值，从而马上再次触发重写。可以通过适当调大 `auto-aof-rewrite-percentage` 来避免过于频繁的重写。
...
![image.png](https://kmk1132-obs-1370539359.cos.ap-guangzhou.myqcloud.com/20250724213619393.png)


比如，添加节点的命令：
![image.png](https://kmk1132-obs-1370539359.cos.ap-guangzhou.myqcloud.com/20250724213638630.png)
...
![image.png](https://kmk1132-obs-1370539359.cos.ap-guangzhou.myqcloud.com/20250724214419086.png)
