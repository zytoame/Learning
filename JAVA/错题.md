...
- 静态代理：由程序员创建代理类或特定工具自动生成源代码再对其编译，在程序运行前代理类的 .class 文件就已经存在了。从 JVM 层面来说， **静态代理在编译时就将接口、实现类、代理类这些都变成了一个个实际的 class 文件。**
	- 静态代理服务于单个接口，我们来考虑实际工程中的一个例子，现在已经有业务代码实现一个增删功能，原有的业务代码由于仍有大量程序无法改变，现在新增需求，即以后每执行一个方法输出一个日志。我们不改变原有代码而添加一个代理来实现：
	```java
	class DateServiceProxy implements DateService {
	    // 问题所在：直接依赖具体实现类 DateServiceImplA
	    DateServiceImplA server = new DateServiceImplA();
	    
	    // 方法实现也直接调用具体类的方法
	    @Override
	    public void add() {
	        server.add();  // 硬编码调用 DateServiceImplA
	        System.out.println("程序执行add方法，记录日志.");
	    }
	}
	```
	- 使用接口依赖：这种方式必须要求客户端传入一个具体的实现类，**这样客户就必须要获得具体目标对象实例，目标对象就直接暴露在访问对象面前了**，对于某些情况这是不可接受的，
		```java
		class DateServiceProxy implements DateService {
		    // 改为依赖接口，而不是具体实现
		    private DateService target;
		    // 通过构造器注入具体的业务对象
		    public DateServiceProxy(DateService target) {
		        this.target = target;
		    }
		    @Override
		    public void add() {
		...
		```
- JDK动态代理：在程序运行时，运用反射机制动态创建而成。**从 JVM 角度来说，动态代理是在运行时动态生成类字节码，并加载到 JVM 中的。** 在 Java 动态代理机制中 `InvocationHandler` 接口和 `Proxy` 类是核心。
	...
	class LoggingHandler implements InvocationHandler {
	    ...
	    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
	        ...
	        System.out.println("程序执行" + method.getName() + "方法，记录日志.");
	        return result;
	    }
	    
	    // 创建代理对象，Proxy类newProxyInstance（）方法
	    public static Object createProxy(Object target) {
	        return Proxy.newProxyInstance(
	            target.getClass().getClassLoader(), // 目标类的类加载器
	            target.getClass().getInterfaces(),  // 代理需要实现的接口，可指定多个
	            new LoggingHandler(target)          // 代理对象对应的自定义 InvocationHandler
	        );
	    }
	}
	...
	public class Test {
	    public static void main(String[] args) {
	        // 动态代理任何实现DateService接口的类
	        DateService serviceA = (DateService) LoggingHandler.createProxy(new DateServiceImplA());
	        DateService serviceB = (DateService) LoggingHandler.createProxy(new DateServiceImplB());
...
# ArrayList和Linkedlist，有十万数据要存储，使用哪个合适？
选择 `ArrayList` 还是 `LinkedList` 取决于你对这 **十万条数据** 的 **主要操作**。

简单来说，**在绝大多数情况下，`ArrayList` 是更优的选择**。下面我们进行详细分析。

### 核心结论

*   **优先选择 `ArrayList`**。它对内存的利用更高效（没有节点指针开销），并且基于数组实现，可以利用 CPU 缓存局部性原理，访问速度极快。对于遍历、随机访问等操作，性能远超 `LinkedList`。
*   **仅在特定场景下考虑 `LinkedList`**。即你需要频繁在 **列表中间** 进行 **插入和删除** 操作，并且这些操作不是通过迭代器进行的。

---

### 详细对比与分析

| 特性 | ArrayList | LinkedList | 十万数据场景分析 |
| :--- | :--- | :--- | :--- |
| **底层结构** | 动态数组 | 双向链表 | **`ArrayList` 胜出**。数组结构紧凑，内存连续。 |
| **随机访问** | **极快 O(1)** | **慢 O(n)** | **`ArrayList` 完胜**。如果你需要频繁使用 `list.get(i)`，`ArrayList` 是唯一选择。`LinkedList` 需要从头或尾遍历。 |
| **头部插入/删除** | **慢 O(n)**<br>(需要移动后续元素) | **快 O(1)** | **`LinkedList` 胜出**。但场景较少，比如用作栈或队列。 |
| **尾部插入/删除** | **快 O(1)**<br>(摊销时间，扩容时除外) | **快 O(1)** | **平手**。两者都很快。`ArrayList` 在偶尔扩容时会有一次数组拷贝的成本。 |
| **中间插入/删除** | **慢 O(n)**<br>(需要移动元素) | **快 O(1)**<br>**但前提是**：你已经定位到了要操作的节点 | **`LinkedList` 理论上胜出**。但注意，定位节点本身是 O(n) 的操作。如果你用 `list.add(index, element)`，它需要先遍历到那个位置。只有使用 `ListIterator` 在迭代过程中进行插入删除，`LinkedList` 的优势才能体现。 |
| **内存占用** | **更小**<br>只存储数据和数组容量 | **更大**<br>每个元素都被包装成Node对象，有前后指针 | **`ArrayList` 胜出**。十万个数据，`LinkedList` 会产生十万个Node对象，内存开销（指针、对象头）远大于 `ArrayList`。 |

---

### 场景模拟与选择

#### 场景一：数据存储后，主要是遍历和随机读取
例如：从数据库查询出十万条记录展示在页面上（分页），或需要根据索引快速查找某些项。

*   **选择：`ArrayList`**
*   **理由：** `get(i)` 和 `foreach` 遍历的速度极快。内存占用小。

#### 场景二：频繁在列表开头插入数据
例如：实现一个消息接收器，新消息总是放在列表最前面。

*   **选择：`LinkedList`**
*   **理由：** `addFirst()` 或 `offerFirst()` 操作是 O(1)，而 `ArrayList` 每次都是 O(n)。

#### 场景三：频繁在列表中间进行插入和删除，并且使用迭代器
例如：实现一个待办事项列表，你使用一个 `ListIterator` 来遍历，并随时根据条件添加或删除任务。

*   **选择：`LinkedList`**
*   **理由：** 使用 `ListIterator.add()` 和 `ListIterator.remove()` 是 O(1) 操作，性能最好。

#### 场景四：用作栈、队列或双端队列 (Deque)
*   **选择：** 对于栈和队列，**优先考虑 `ArrayDeque`**，它比 `LinkedList` 性能更好，内存占用更小。只有在需要同时实现 `List` 和 `Deque` 接口的特定情况下，才用 `LinkedList`。

---

### 总结与建议

1.  **默认选择 `ArrayList`**： 对于存储十万条数据，如果你不确定该怎么选，或者你的操作是混合型的（偶尔读，偶尔写），那么 `ArrayList` 几乎总是正确的答案。它的平均性能和内存效率远高于 `LinkedList`。

2.  **谨慎选择 `LinkedList`**： 只有在你非常确定你的应用存在 **大量的、非基于迭代器的中间位置插入删除**，并且这个性能瓶颈通过 profiling 工具证实后，才考虑使用 `LinkedList`。在实践中，这种场景非常少见。

3.  **考虑其他数据结构**： 你的需求可能更适合其他数据结构。
    *   如果需要 **键值对** 访问，用 `HashMap`。
    *   如果需要 **去重** 和快速存在性检查，用 `HashSet`。
    *   如果需要 **队列/栈** 功能，用 `ArrayDeque`。
    *   如果需要 **排序**，用 `TreeMap` 或 `TreeSet`。

**最终建议：**
对于你的问题“有十万数据要存储”，在没有更多上下文的情况下，**请毫不犹豫地选择 `ArrayList`**。
# 排序
